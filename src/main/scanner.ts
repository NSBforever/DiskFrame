...
const SKIP_DIRS = ['windows', 'program files', 'program files (x86)', '$recycle.bin', 'system volume information', 'programdata', 'node_modules', '.git']
...
const docExts = ['.pdf', '.docx', '.doc', '.txt', '.xlsx', '.xls', '.pptx', '.ppt', '.csv', '.m', '.py', '.js', '.ts']
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        try {
          ...
          db.prepare(`INSERT OR IGNORE INTO files (path, name, ext, size, date, year, month, lat, lng, drive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
